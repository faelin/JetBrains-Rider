...
parse_git_branch() {
	staged_count=$(git status --no-optional-locks --porcelain=1 2> /dev/null | grep -e '^[^?! ]' | wc -l | sed -E "s/([0-9]+``)/<\1>/; s/\<0\>//" | xargs)
	unstag_count=$(git status --no-optional-locks --porcelain=1 2> /dev/null | grep -e '^.[^ ]'  | wc -l | sed -E "s/([0-9]+)/(\1)/; s/\(0\)//" | xargs)
...
print_git_branch() {
	parse_git_branch | sed -E "s/^\[([^~+ ]+)(~[0-9]+)?(\+[0-9]+)?( )?(<[0-9]+>)?(\([0-9]+\))?\]$/$(echo -e " ${STYLE_BOLD}${COLOR_YELLOW}[\1${COLOR_RED}\2${COLOR_GREEN}\3\4${COLOR_CYAN}\5${COLOR_PINK}\6${COLOR_YELLOW}]${COLOR_RESET}")/"
}
